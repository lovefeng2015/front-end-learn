/*
 * @Author: yhf 
 * @Date: 2021-11-Mo 11:47:48 
 * @Last Modified by:   yhf 
 * @Last Modified time: 2021-11-Mo 11:47:48 
 */
/**
 * @param {number[]} nums
 * @param {number} k
...
//可以跑一遍把频次拿出来先排个序，找到第k大的频次 target，再跑一遍map拿到所有大于等于 target的值。题目有说明答案集合唯一。
var topKFrequent = function (nums, k) {
    ...
    let arr = counts.sort((a, b) => b - a);
    let kcount = arr[k - 1];
    let res = [];
    for (const [key, vaule] of map.entries()) {
        if(res.length>=k){
            break;
        }
        if (vaule >= kcount) {
...
var topKFrequent = function (nums, k) {
    ...
    let arr = newArr.sort((a, b) => b[1] - a[1]);
    let res = [];
    for (let i = 0; i < k; i++) {
        res.push(arr[i][0]);
    }
